<?php
/*
  Plugin Name: Cute Text
  Plugin URI: Plugin URI: http://themeforest.net/user/torbara/
  Description: Display widget of Text Category
  Author: ovvod
  Version: 1.0.0
  Author URI:
 */

class CuteText extends WP_Widget {
    
    function __construct() {
        $widget_ops = array('classname' => 'CuteText', 'description' => 'Display posts as Text');
        parent::__construct('CuteText', 'Cute Text', $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        ?>
                <div data-uk-slideset="{default: 1}" class="tm-bible-slideset">
                    <div class="tm-text-slider-widget">
                        <ul class="uk-grid uk-slideset uk-grid-collapse">
                            <?php
                            foreach ($list as $post) { ?>
                            <li>
                                <div class="uk-text-center text-shadow"><?php echo wp_trim_words($post->post_content, 13); ?></div>
                            </li>
                            <?php    
                            }
                            ?>
...
add_action('widgets_init', create_function('', 'return register_widget("CuteText");')); ?>